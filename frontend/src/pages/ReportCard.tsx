...
export const ReportCard = () => {
  ...
  const reportCardData = useMemo(() => {
    ...
    return {
      ...
      overallRemarks: getRemarks(cumulativeAverage),
    };
  ...
  useEffect(() => {
    const fetchComments = async () => {
      if (!selectedStudent || !reportCardData?.terms[currentTermIndex]?.termId) {
        setTeacherComments([]);
        return;
      }
      try {
        const termId = reportCardData.terms[currentTermIndex].termId;
        const response = await api.get(`/class-teacher/comments/${selectedStudent}?termId=${termId}`);
        setTeacherComments(response.data?.data || []);
      } catch (error) {
        setTeacherComments([]);
      }
    };
    fetchComments();
  }, [selectedStudent, currentTermIndex, reportCardData]);
    ...
    <div className="space-y-6 print:space-y-2">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <SearchBar onSearch={setSearchTerm} placeholder="Name or admission number..." />
            </div>
            <div>
      ...
      {gradesLoading ? (
        ...
        <div className="bg-white rounded-lg shadow-lg print:shadow-none overflow-hidden">
          ...
          <div className="border-b print:hidden">
            <div className="flex items-center justify-between px-4">
              <button onClick={prevTerm} disabled={currentTermIndex === 0}
                className="p-2 text-gray-500 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">
                <ChevronLeftIcon className="w-5 h-5" />
              </button>
              <div className="flex">
                {reportCardData.terms.map((term, index) => (
                  <button key={term.termId} onClick={() => setCurrentTermIndex(index)}
                    className={`px-6 py-3 text-sm font-medium transition-colors ${currentTermIndex === index ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500 hover:text-gray-700"}`}>
                    {term.termName}
                  </button>
                ))}
              </div>
              <button onClick={nextTerm} disabled={currentTermIndex === reportCardData.terms.length - 1}
                className="p-2 text-gray-500 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {/* TEACHER REMARKS SECTION WITH SAVED COMMENTS */}
          <div className="p-6 border-t bg-gray-50 print:p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                ...
                <Badge variant={reportCardData.cumulativeGrade === "A" ? "success" : reportCardData.cumulativeGrade === "B" || reportCardData.cumulativeGrade === "C" ? "warning" : "danger"} className="mt-2">
              ...
              <div className="col-span-2">
                ...
                {teacherComments.length > 0 ? (
                  <div className="space-y-2">
                    {teacherComments.map((comment: any, index: number) => (
                      <div key={index} className="bg-white p-3 rounded border border-gray-200">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="info" size="sm">
                            {comment.type?.replace(/_/g, ' ') || 'General'}
                          </Badge>
                          <span className="text-xs text-gray-400">
                            {new Date(comment.createdAt).toLocaleDateString()}
                          </span>
                        </div>
                        <p className="text-gray-800 italic">"{comment.comment}"</p>
                      </div>
                    ))}
                  </div>
                ) : (
        ...
        <div className="text-center py-12 bg-white rounded-lg">
          ...
          <p className="text-gray-500">No grades have been entered for this student in the selected session.</p>
        </div>
      ) : (
        <div className="text-center py-12 bg-white rounded-lg">
          <AcademicCapIcon className="w-16 h-16 text-gray-300 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-gray-900 mb-2">Select a Student</h3>
          <p className="text-gray-500">Choose a student from the dropdown above to generate their report card.</p>
        </div>
      )}
    </div>
  );
          ...
          <tbody>
            {term.subjects.map((subject, idx) => (
              <tr key={subject.subjectId} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 px-4 py-2 text-center text-sm">{subject.ca !== null ? subject.ca : "-"}</td>
                <td className="border border-gray-300 px-4 py-2 text-center text-sm">{subject.exam !== null ? subject.exam : "-"}</td>
                <td className="border border-gray-300 px-4 py-2 text-center text-sm font-semibold">{subject.total !== null ? subject.total : "-"}</td>
                <td className="border border-gray-300 px-4 py-2 text-center">
                  {subject.grade ? <Badge variant={subject.grade === "A" ? "success" : subject.grade === "B" || subject.grade === "C" ? "warning" : "danger"}>{subject.grade}</Badge> : "-"}
                </td>
                <td className="border border-gray-300 px-4 py-2 text-sm">{getSubjectRemarks(subject.total)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="bg-gray-100 font-semibold">
              <td colSpan={4} className="border border-gray-300 px-4 py-2 text-right">Term Average:</td>
              <td className="border border-gray-300 px-4 py-2 text-center">{term.termAverage.toFixed(1)}%</td>
              <td className="border border-gray-300 px-4 py-2 text-center">
                <Badge variant={term.termGrade === "A" ? "success" : term.termGrade === "B" || term.termGrade === "C" ? "warning" : "danger"}>{term.termGrade}</Badge>
              </td>
              <td className="border border-gray-300 px-4 py-2 text-sm">{getTermRemarks(term.termAverage)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
